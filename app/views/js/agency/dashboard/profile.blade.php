...
var questionsCount = '{{ count($questions) }}';
var questionnaires = [];
var questionnairesCount = '{{ count($questionnaires) }}';
var templates = [];
var templatesCount = '{{ count($templates) }}'
...
var ProfileQuestionsTableEditable = function () {
    ...
    var handleTable = function () {
        ...
        function saveRow(oTable, nRow) {
            ...
            var companyId = '{{ $agency->id }}';
            var question = jqInputs[0].value;
            var time = jqInputs[1].value;

            $.ajax({
                url: "{{ URL::route('agency.profile.async.saveQuestion') }}",
                dataType : "html",
                type : "POST",
                data : {agency_id: companyId, question: question, time: time},
        ...
        table.on('click', '.delete', function (e) {
            ...
            $.ajax({
                url: "{{ URL::route('agency.profile.async.deleteQuestion') }}",
                dataType : "json",
                type : "POST",
                data : {agency_id: companyId, question: question},
                success : function(data) {
                    $('div#select_questions_table_box').html(data);
                    questionsCount --;
                    QuestionsTableManaged.init();
...
var TemplatesTableAdvanced = function () {
    ...
    var initTable = function () {
        ...
        table.on('click', '.delete', function (e) {
            ...
            $.ajax({
                url: "{{ URL::route('agency.profile.async.deleteVITemplate') }}",
                dataType : "json",
                type : "POST",
...
var QuestionnaireTableAdvanced = function () {
    ...
    var initTable3 = function () {
        ...
        table.on('click', '.delete', function (e) {
            ...
            $.ajax({
                url: "{{ URL::route('agency.profile.async.deleteQuestionnaire') }}",
                dataType : "json",
                type : "POST",
...
function saveVITemplate(obj) {
    ...
    if (title == '') {
        $('p#js-p-vi-template-warnning').html('{{ trans('company.msg_24') }}');
        $('div#vi-template-alert-box').removeClass('hidden');
        return;
    }

    if (description == '') {
        $('p#js-p-vi-template-warnning').html('{{ trans('company.msg_25') }}');
        $('div#vi-template-alert-box').removeClass('hidden');
        return;
    }
    ...
    $.ajax({
        url: "{{ URL::route('agency.profile.async.saveVITemplate') }}",
        dataType : "json",
        type : "POST",
        data : {agency_id: companyId, title: title, description: description, template_id: template_id},
        success : function(data) {
            $('div#viTemplateModal').modal('hide');
            $('div#vi_templates_table_box').html(data.table);
            templates = data.questionnaires;
...
function saveQuestionnaire(obj) {
    ...
    $.ajax({
        url: "{{ URL::route('agency.profile.async.saveQuestionnaire') }}",
        dataType : "json",
        type : "POST",
        data : {agency_id: companyId, questions: questions, title: title, questionnaire_id: questionnareId},
        success : function(data) {
...
<?php
	foreach ($agency->followCompanies as $fCompany) {?>
...
$(document).ready(function() {
	...
	<?php if ($agency->followCompanies()->count() == 0) {?>
	onAddFollowCompany('');
	<?php }?>


	var lat = '<?php echo $agency->lat?>';
	var lng = '<?php echo $agency->long?>';
