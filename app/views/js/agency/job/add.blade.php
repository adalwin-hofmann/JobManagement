...
var TableManaged = function () {
    ...
    var initTable1 = function () {
        ...
        var table = $('#client_company_table');
        ...
        var tableWrapper = jQuery('#client_company_table_wrapper');
...
function addCompanyToList(companyId) {
    var listValue = $('input#share_companies').val();

    if (listValue.indexOf(companyId) == -1) {
        if (listValue == '') {
            listValue = companyId;
        }else {
            listValue = ',' + companyId;
    ...
    $('input#share_companies').val(listValue);
}

function removeCompanyFromList(companyId) {
    var listValue = $('input#share_companies').val();

    if (listValue.indexOf(companyId) >= 0) {
        var newValue = '';
        var ids = listValue.split(',');
        for (var i = 0; i < ids.length; i ++) {
            if (ids[i] == companyId) continue;
