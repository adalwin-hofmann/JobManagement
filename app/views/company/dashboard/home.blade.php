    ...
    <div class="container">
    	<div class="row">
			...
			<div class="col-sm-12 margin-top-xs dashboard-container padding-top-normal padding-bottom-sm">
				...
				<form class="form-horizontal" method="post" action="{{ URL::route('company.dashboard') }}" id="search-form">
									...
									<div class="number">
										<?php 
											...
											if ($company->jobs()->get()->count() > 0) {
                                                foreach ($company->jobs()->where('created_at', '>', $startDate.' 00:00:00')->where('created_at', '<=', $endDate.' 23:59:59')->where('is_active', 1)->where('is_finished', 1)->get() as $job) {
                                                    $apply_count = $apply_count + $job->applies()->count();
                                                    $hint_count = $hint_count + $job->hints->count();
                                                }
                                            }else{
                                                $apply_count = $company->parent->companyApplies()->where('created_at', '>', $startDate.' 00:00:00')->where('created_at', '<=', $endDate.' 23:59:59')->get()->count();
                                                $hint_count = 0;
                                            }
										?>
										{{ $apply_count }}										
					...
					<div class="col-sm-10 col-sm-offset-1">
						<div class="portlet">
							...
							<div class="portlet-body">
								...
								@if ($apply_count > 0)
								    @if ($company->jobs()->get()->count() > 0)
                                        @foreach ($company->jobs()->where('created_at', '>', $startDate.' 00:00:00')->where('created_at', '<=', $endDate.' 23:59:59')->where('is_active', 1)->where('is_finished', 1)->orderBy('created_at', 'DESC')->limit(10)->get() as $job)
                                        ...
                                        @endforeach
                                    @else
                                        @foreach($company->parent->companyApplies()->where('created_at', '>', $startDate.' 00:00:00')->where('created_at', '<=', $endDate.' 23:59:59')->get()  as $apply)
                                            <div id = "div_apply">
                                                <div class="row">
                                                    <div class="col-sm-2 text-center">
                                                        <img style="width: 50px; height: 50px;" src="{{ HTTP_PHOTO_PATH.$apply->user->profile_image }}" class="img-circle">
						            	...
						            	@foreach ($company->jobs()->where('created_at', '>', $startDate.' 00:00:00')->where('created_at', '<=', $endDate.' 23:59:59')->where('is_active', 1)->where('is_finished', 1)->get() as $job)
											@foreach ($job->hints as $hint)
											    <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ URL::route('company.job.view', $hint->job->slug)  }}" class="btn btn-sm btn-info">
	    		...
	    		<div class="portlet box">
	    			...
	    			<div class="portlet-body">
				        ...
				        <table class="table table-striped table-bordered table-hover dataTable no-footer">
				            ...
				            <tbody>
				                @foreach ($company->members as $key => $value)
				                    <tr>
				                        <td>{{ $key + 1 }}</td>
				                        <td>{{ $value->name }}</td>
				                        <td>{{ $value->email }}</td>
				                        ...
				                        <td>
				                            <a data-url="{{ URL::route('company.user.delete', $value->id)  }}" class="btn btn-sm btn-danger" id="js-a-delete">
...
@section('custom-scripts')
    @include('js.company.dashboard.home')
@stop