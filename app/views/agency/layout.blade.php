            ...
            <div class="header-navigation pull-right font-transform-bitter company-menu-bar">
                <ul class="nav nav-pills nav-top">
                    @if (Session::has('agency_id'))
                    	...
                    	<li class="{{ ($pageNo == 10) ? 'active' : ''}}"><a href="{{ URL::route('agency.company.index') }}">{{ trans('menu.companies') }}</a></li>
                    @else
                        <li class="{{ ($pageNo == 98) ? 'active' : ''}}">
                        	<a href ="{{ URL::route('agency.auth.login') }}">{{ trans('menu.sign_in') }}</a>
                        </li>
                        <li class="{{ ($pageNo == 99) ? 'active' : ''}}">
                        	<a href="{{ URL::route('agency.auth.signup') }}">{{ trans('menu.register') }}</a>
                        </li>
                    @endif
                </ul>
            </div>
