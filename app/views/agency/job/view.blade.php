	...
	<div class="container" style="background-color: #FFF;">
		...
		<div class="row margin-top-normal">
			<div class="col-sm-6 company-job-title">
				{{ $job->name }}
				@if ($agency->agencyShares()->where('job_id', $job->id)->get()->count() > 0)
				    @foreach ($agency->agencyShares()->where('job_id', $job->id)->get() as $item)
				        &nbsp;<span class="label" style="background-color: #009cff">Shared To <a href="{{ URL::route('user.company.view', $item->company->slug) }}" target="_blank" class="pointer color-white">{{ $item->company->name }}</a></span>
				    @endforeach
				@endif
			</div>
			...
			<div class="col-sm-6">
                ...
                @if ($agency->agencyShares()->where('job_id', $job->id)->get()->count() > 0)
                    @foreach ($agency->agencyShares()->where('job_id', $job->id)->get() as $item)
                        <div class="row margin-top-sm">
                            <div class="col-sm-12">
                                <label>Note By {{ $item->company->name }}:</label>
                            </div>
                        </div>
                        <div class="row margin-top-xxs">
                            <div class="col-sm-12">
                                <textarea class="form-control" rows="8" readonly>{{ $item->noteByCompany($item->company->id) }}</textarea>
                            </div>
                        </div>
                    @endforeach
                @endif
			...
			<?php foreach($hints as $hint){?>
				<div id = "div_hint">
					<div class="row">
						...
						<div class="col-sm-4 text-center" style="margin-top: 4px;">
							<a class="btn btn-sm btn-danger <?php if ($hint->job->company->id != $agency->id) {?> disabled <?php }?>" id="js-a-hint-reject" data-id="{{ $hint->id }}">
			                	<i class="fa fa-times"></i> Reject
			                </a>
						</div>
					</div>
