...
class UserController extends \BaseController {
    ...
    public function deleteMember($id) {
        ...
        return Redirect::route('company.dashboard')->with('alert', $alert);
    }
    ...
    public  function findPeople() {
        if (!Session::has('company_id')) {
            ...
            return View::make('company.user.find')->with($param);
        }
    }
    ...
    public  function candidates() {
        if (!Session::has('company_id')) {
            ...
            return View::make('company.user.candidates')->with($param);
        }
    }
    ...
    public  function appliedPeople() {
        if (!Session::has('company_id')) {
            ...
            $param['candidates'] = CompanyCandidatesModel::whereIn('company_id', $company->companyIds())->get();

            $param['questionnaires'] = QuestionnairesModel::where('company_id', $company->id)->orWhere('by_admin', 1)->get();
            $param['viTemplates'] = CompanyVITemplateModel::where('company_id', $company->id)->orWhere('by_admin', 1)->get();

            return View::make('company.user.applied')->with($param);
        }
    }
    ...
    public function sharedPeople() {
        if (!Session::has('company_id')) {
            ...
            return View::make('company.user.shared')->with($param);
        }
    }
    ...
    public function asyncSaveShareNotes() {
        ...
        if (Session::has('company_id')) {
            ...
            $agencyShares = AgencyShareModel::where('company_id', $companyId)->where('user_id', $userId)->get();

            foreach($agencyShares as $agencyShare) {
                if (CompanyShareNoteModel::where('share_id', $agencyShare->id)->where('company_id', $companyId)->get()->count() > 0) {
                    $note = CompanyShareNoteModel::where('share_id', $agencyShare->id)->where('company_id', $companyId)->firstOrFail();
                    $note->note = $notes;
                    $note->save();
                } else {
                    $note = new CompanyShareNoteModel;

                    $note->share_id = $agencyShare->id;
                    $note->company_id = $companyId;
                    $note->note = $notes;
    ...
    public function asyncRequestFeedback() {
        if (Session::has('company_id')) {
            ...
            Queue::push('\SH\Queue\CompanyRequestFeedbackMessage', ['user_id' => $userId, 'company_id' => Session::get('company_id'), 'member_id' => $memberId, 'message_data' => $message_data] );
    ...
    public function asyncSaveRate() {
        if (Session::has('company_id')) {
            ...
            $count = CompanyUserScoreModel::where('company_id', Session::get('company_id'))->where('user_id', $userId)->get()->count();

            if ($count > 0) {
                CompanyUserScoreModel::where('company_id', Session::get('company_id'))->where('user_id', $userId)->delete();
            }

            $userScore = new CompanyUserScoreModel;
            $userScore->company_id = Session::get('company_id');
    ...
    public function asyncSendMessage() {
        if (Session::has('company_id')) {
            ...
            Queue::push('\SH\Queue\CompanyUserApplyMessage', ['user_id' => $userId, 'company_id' => Session::get('company_id'), 'message_data' => $message_data] );
            ...
            return Response::json(['result' => 'success', 'msg' => 'Message has been sent successfully.', 'messageView' => View::make('company.user.ajaxMessageView')->with($param)->__toString()]);
        } else {
            return Response::json(['result' => 'fail', 'msg' => 'Invalid Request.']);
        }         
    }
    ...
    public function asyncSendInvite() {
        if (Session::has('company_id')) {
            ...
            Queue::push('\SH\Queue\CompanySendInviteMessage', ['user_id' => $userId, 'company_id' => Session::get('company_id'), 'job_id' => $jobId] );
    ...
    public function asyncUserView() {
        ...
        if (Session::has('company_id')) {
            ...
            return Response::json(['result' => 'success', 'userView' => View::make('company.user.ajaxView')->with($param)->__toString()]);
        } else {
            return Response::json(['result' => 'fail', 'msg' => 'Invalid Request.']);
        }
    }
    ...
    public function asyncAddCandidate() {
        ...
        if (Session::has('company_id')) {
            ...
            if (UserModel::where('email', Input::get('email'))->get()->count() > 0) {
                ...
                $companyCandidates->company_id = Session::get('company_id');
                ...
                if ($note != '') {
                    ...
                    $userNote->company_id = Session::get('company_id');
                    $userNote->notes = $note;
                ...
                Queue::push('\SH\Queue\CompanyAddCandidateNRMessage', ['user_id' => $user->id, 'company_id' => Session::get('company_id')] );
    ...
    public function asyncCheckAvailableJobs() {
        ...
        if (Session::has('company_id')) {
            ...
            return Response::json(['result' => 'success', 'msg' => 'Candidate has been added successfully.', 'jobsView' => View::make('company.user.ajaxAvailableJobs')->with($param)->__toString()]);
        } else {
            return Response::json(['result' => 'fail', 'msg' => 'Invalid Request.']);
        }
